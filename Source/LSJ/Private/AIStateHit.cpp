...
void UAIStateHit::Enter ( UAICharacterAnimInstance* pAnimInstance )
{
	...
	{
		//FVector Direction = owner->GetActorLocation ( ) - owner->aOpponentPlayer->GetActorLocation ( );
		...
		// 반전된 벡터에 강도 적용
		FVector LaunchVelocity = owner->GetCapsuleComponent()->GetForwardVector() *-1.f * attackInfoArray[0].KnockBackDirection.X;
